...
public class MurderESPRenderer {
    ...
    public void onRenderWorldLast(RenderWorldLastEvent event) {
        ...
        // Only render if there are targets to highlight
        if (!MurderDetectionHandler.hasAnyTargets()) {
            return;
        }
        ...
        for (EntityPlayer player : mc.theWorld.playerEntities) {
            ...
            if (TitleHandler.isPlayerMurderer()) {
                ...
                if (detectives.contains(playerName)) {
                    // Check if this is the first bow holder (repeatable bow)
                    if (playerName.equals(firstBowHolder)) {
                        // Gold ESP for first bow holder (repeatable bow)
